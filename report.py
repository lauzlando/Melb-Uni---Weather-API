...
def fetch_all_cities(cities):
    ...
    "weather": {....}: Dict returned by get.weather
    """
...
def update_log(log_path, new_results):
    """
    Append new weathe results to a JSON log file.
...
def summarise_log(log_path):
    ...
    Assumptions:
    - If log is emppty or missing, returne all fields with None or empty values. 
...
def verify_correctness():
    ...
    - Does not require internet - checks structure only
    ...
    record("city" in results[0], "fetch_all_cities includes 'city'")
    record("fetched_at" in results[0], "fetch_all_cities includes 'fetched_at'")
    record("weather" in results[0], "fetch_all_cities includes 'weather'")
...
<h1>Weather Report - Generated {report_time}</h1>
